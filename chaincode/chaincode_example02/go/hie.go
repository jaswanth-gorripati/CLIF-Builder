...
func (t *HIEChaincode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	} else if function == "getAllIOfiles" {

		// Query Hashes of all the uploaded files for a particular MSH key
		return t.getAllIOfiles(stub, args)
	}
...
func getDestOrg(stub shim.ChaincodeStubInterface, MSHKEY string) (string, string, string) {
	...
	if err != nil {
		...
		if err != nil {
			return "", "", "Failed to unmarshal iterative string"
		}
	} else {
		return "", "", "no details found for " + MSHKEY
	}
	return destOrg.DestOrganization, destOrg.SrcOrganizationID, ""
}
...
func isDS(stub shim.ChaincodeStubInterface, mshFilesbytes []byte, MSHKEY string, invoker string) (bool, pb.Response) {
	...
	if destID == "" {
		return false, shim.Error(errResp)
	}
...
func (t *HIEChaincode) addMSH(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	destOrg, srcOrg, _ := getDestOrg(stub, HL7MshDetails.MSHKEY)
	if destOrg != "" {
...
func (t *HIEChaincode) adddestorg(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	DestOrgDetails.Doctype = "DestOrg"
...
func (t *HIEChaincode) addMshFiles(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	// }
	// MSHFILES composite kay creation
	indexName := "MshFilesID~MSHKEY~Filetype~Hash"
	mshFilesCK, err := stub.CreateCompositeKey(indexName, []string{mshfilesList.MshKey + "_" + "MSHfiles", mshfilesList.MshKey, mshfilesList.FileType, mshfilesList.Hash})
	if err != nil {
		return shim.Error(err.Error())
	}
	value := []byte{0x00}
	err = stub.PutState(mshFilesCK, value)
	if err != nil {
		return shim.Error("failed to put MSHfiles Composite key , error: " + err.Error())
	}
	//	Getting Destination organisation of the Msh Key to send an Event to the Destination Organisation about the file upload
	destID, _, _ := getDestOrg(stub, mshfilesList.MshKey)
...
func (t *HIEChaincode) getAllMshFiles(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	fmt.Println("requuest for GetMshFies by " + invoker + "  for MSHKEY: " + args[0])
	mshKey := args[0]
	indexName := "MshFilesID~MSHKEY~Filetype~Hash"
	mshFiles := MSHFiles{}
	mshFilesIterator, err := stub.GetStateByPartialCompositeKey(indexName, []string{mshKey + "_" + "MSHfiles"})
	if err != nil {
		return shim.Error(err.Error())
	}
	defer mshFilesIterator.Close()
	var i int
	mshFiles.MSHKEY = mshKey
	for i = 0; mshFilesIterator.HasNext(); i++ {
		responseRange, err := mshFilesIterator.Next()
		if err != nil {
			return shim.Error(err.Error())
		}
		...
		//returnedMSH := compositeKeyParts[0]
		returnedFileType := compositeKeyParts[2]
		returnedHash := compositeKeyParts[3]
		eMsg := ""
		eMsg, mshFiles = getMshFileType(mshFiles, returnedFileType, returnedHash)
		if eMsg != "" {
			return shim.Error("Dont have File type " + returnedFileType + " in the MSH files list")
		}
		fmt.Println(mshFiles)
		//mshFiles = mshFiles
	}
	mshFilesbytes, err := json.Marshal(mshFiles)
	if err != nil {
		return shim.Error("connot return Msh File bytes ,Error: " + err.Error())
	}
...
//
////	 getAllIOfiles, query  couchdb using the string
//

func (t *HIEChaincode) getAllIOfiles(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	if len(args) != 1 {
		return shim.Error("Incorrect number of arguments. Expecting inbound or outbound string")
	}
	...
	queryString := ""
	if args[0] == "inbound" {
		queryString = "{\"selector\":{\"doctype\":\"DestOrg\",\"desOrganizationID\":\"" + invoker + "\"}}"
	} else if args[0] == "outbound" {
		queryString = "{\"selector\":{\"doctype\":\"DestOrg\",\"srcOrganizationID\":\"" + invoker + "\"}}"
	} else {
		return shim.Error("cannot process request , for " + args[0] + "")
	}
	...
	for resultsIterator.HasNext() {
		...
		destOrgFile := DestOrg{}
		err = json.Unmarshal(queryResponse.Value, &destOrgFile)
		if err != nil {
			return shim.Error(err.Error())
		}
		mshBytes, err := stub.GetState(destOrgFile.MSHKEY)
		MshFile := Hl7Msh{}
		err = json.Unmarshal(mshBytes, &MshFile)
		if err != nil {
			return shim.Error(err.Error())
		}
		queryResults.DestOrg = append(queryResults.DestOrg, destOrgFile)
		queryResults.MSHinfo = append(queryResults.MSHinfo, MshFile)
	}
	responseBytes, err := json.Marshal(queryResults)
	if err != nil {
		return shim.Error(err.Error())
	}
