...
function initPeerVars {
   ...
   #export CORE_VM_DOCKER_HOSTCONFIG_NETWORKMODE=${COMPOSE_PROJECT_NAME}_${EXTERNAL_NETWROK_NAME}
   if [ ${DOCKER_NETWORK_TYPE} == "DOCKER-COMPOSE" ]; then
    export CORE_VM_DOCKER_HOSTCONFIG_NETWORKMODE=clif_${EXTERNAL_NETWROK_NAME}
    else
    export CORE_VM_DOCKER_HOSTCONFIG_NETWORKMODE=${EXTERNAL_NETWROK_NAME}
   fi
